...
$(document).ready(function() {
    ...
    $('#AgriculteurID').select2({
        ...
        ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
            url: function(){return "{{ URL::to('agriculteur/select2/ajax') }}"; },
            dataType: 'json',
            quietMillis: 250,
        ...
        @if ($agriculteurJson = Session::get('agriculteurJson'))
        initSelection: function (element, callback) {
            var agriculteurJson = {{ $agriculteurJson }};
            if (agriculteurJson) {
                callback(agriculteurJson);
            }
        },
        @endif
    ...
    $('#ExploitationID').select2({
        ...
        @if ($exploitationJson = Session::get('exploitationJson'))
        initSelection: function (element, callback) {
            var exploitationJson = {{ $exploitationJson }};
            if (exploitationJson) {
                callback(exploitationJson);
            }
        },
        @endif
    ...
    $('#ProduitID').select2({
        ...
        @if ($produitJson = Session::get('produitJson'))
        initSelection: function (element, callback) {
            var produitJson = {{ $produitJson }};
            if (produitJson) {
                callback(produitJson);
            }
        },
        @endif
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            @if ( $errors->count() > 0 )
                                ...
                                @endforeach
                            @endif
                            {{ Form::open(array('url' => URL::to('production') , 'role' => 'form', 'class' => 'form-horizontal')) }}
                                <div class="form-group @if($errors->first('CampagneAgricoleID') != '')) has-error @endif">
                                    ...
                                    <div class="col-lg-9">
                                    {{ Form::select('CampagneAgricoleID', $campagneAgricoles, Input::old('CampagneAgricoleID'), array('class' => 'form-control')) }}
                                ...
                                <div class="form-group @if($errors->first('ExploitationID') != '')) has-error @endif">
                                    ...
                                    <div class="col-lg-9">
                                        <input type="hidden" class="bigdrop form-control" id="ExploitationID" name="ExploitationID" value="{{Input::old('ExploitationID')}}" />
